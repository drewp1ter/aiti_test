...
import { withCreateProductModal, type WithCreateProductModal } from '@/entities/product'
...
export const ProductList = withCreateProductModal(
	observer(function ProductList({ openCreateProductModal }: WithCreateProductModal) {
			...
			<section className={styles.productList}>
				...
				<div className={styles.content}>
					<div className="flex gap-2">
						...
						<Button className="ml-auto" variant="ghost" variantSize="42" onClick={handleRefresh}>
							<Icon.ArrowsClockwise />
						</Button>
						<Button variant="secondary" variantSize="42" onClick={handleAddProduct}>
							<Button.Prefix>
								<Icon.PlusCircle />
							</Button.Prefix>
							Добавить
								...
								<Table.Row>
									<Table.Head className="cursor-pointer">
										<div className="flex items-center gap-5">
											...
											{sortBy === 'title' && (
												<Icon.CaretLeft className={order === 'asc' ? 'rotate-90' : 'rotate-270'} />
											)}
										</div>
									</Table.Head>
									...
									<Table.Row key={product.id} isSelected={productsStore.isProductSelected(product.id)}>
										<Table.Cell onClick={() => productsStore.selectProduct(product.id)}>
											<ProductCell
												...
												checkbox={
													<Checkbox checked={productsStore.isProductSelected(product.id)} onChange={() => {}} />
												}
											/>
										</Table.Cell>
